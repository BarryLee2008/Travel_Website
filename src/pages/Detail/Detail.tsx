...
// 引入ant design插件
import { Row, Spin, Col, DatePicker, Divider, Typography, Anchor, Menu, Button } from "antd";
import { ShoppingCartOutlined } from '@ant-design/icons'
import { MainLayout } from '../../layouts'
// 引入子组件
import { ProductIntro, Comments } from '../../components'
// 引入css模块
import styles from './Detail.module.css'
// 引入假数据
import { commentMockData } from './mockData'
// 引入slice
import { getDataByFetching } from '../../redux/productDetail/productDetailSlice'
// 引入自定义useSelector和useDispatch
import { useSelector, useDispatch } from '../../redux/hooks'
// 引入添加购物车thunk
import { addshoppingCartAsyncThunk } from '../../redux/shoppingCart/ShoppingCartSlice'
import { type } from "os";
// 引入dispatch
...
export const Detail: React.FC = () => {
    ...
    const handleAddShoppingCart = () => {
        if (jwt && typeof touristRouteId === 'string') {
            dispatch(addshoppingCartAsyncThunk({ jwt: jwt, touristRouteId: touristRouteId }))
        } else {
            navigate('/login')
        }
    }
    // 设置useEffect来处理异步调用API
    // useEffect第一个参数是回调函数，第二个参数是[数据]，表示监控的数据。只要数据变化就会第一个回调函数就会被调用。下面[]表示空表示只会mount后执行一次
    useEffect(() => {
                ...
                <div className={styles["page-content"]}>
                    ...
                    <div className={styles["product-intro-container"]}>
                        <Row>
                            ...
                            {/* 右侧日期选择 */}
                            <Col span={11}>
                            <Button style={{ marginTop: 30, marginBottom: 30, display: 'block' }} type='primary' danger loading={shoppingCartLoading} onClick={handleAddShoppingCart}>添加到购物车</Button>
                                {/* open 表示日日历打开 */}
